...
def test_run_python_version(client):
    ...
    # finished
    assert generic_data['result'] == 'oc login https://do-prd-okp-m0.do.viaa.be:8443 -p "configure_user" -u "configure_pass" --insecure-skip-tls-verify && oc delete jobs syncrator-qas-avo-delta && oc process -f syncrator-openshift/job_template.yaml -p TARGET="avo" -p ENV="qas" -p ACTION_NAME="delta" -p ACTION="delta" -p IS_TAG="latest" -p OPTIONS="-n 1000 -c 1 -api_job_id 4" | oc create -f - && oc logout'
